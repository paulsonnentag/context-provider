...
// and lets the user edit or delete each one. The StateHandle is injected via
// `prop:handle` from the same instance the CommentsProvider was given, so
// additions/removals stream in live; per-doc edits go through the mounted
// sub-handle returned by `state.ref(url)`.
...
>(({ element, handle: state }) => {
  ...
  const onChange = () => setGroups({ ...state.value });
